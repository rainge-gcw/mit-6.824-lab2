 ...
 */
    ...
    class Server {
    public:
        using Handler = std::function<void(const Request &, Response &)>;//句柄

        using ExceptionHandler =
                std::function<void(const Request &, Response &, std::exception_ptr ep)>;//错误时的句柄

        enum class HandlerResponse {//枚举类
            Handled,
            Unhandled,
        };
        ...
        virtual ~Server();//先找到并执行子类的析构函数，然后再执行父类的析构函数，资源正常释放，避免了内存泄漏。
